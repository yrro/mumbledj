...
func (dj *MumbleDJ) OnConnect(e *gumble.ConnectEvent) {
	dj.Client.Self.Move(dj.Client.Channels.Find(dj.Config.DefaultChannel))

	dj.AudioStream = gumble_ffmpeg.New(dj.Client)
	dj.AudioStream.Volume = dj.Config.Volume.DefaultVolume

	dj.Client.Self.SetComment(dj.Config.DefaultComment)
...
func (dj *MumbleDJ) OnDisconnect(e *gumble.DisconnectEvent) {
	if dj.Config.Connection.Retry.Enabled && (e.Type == gumble.DisconnectError || e.Type == gumble.DisconnectKicked) {
		dj.Log.Printf("Disconnected from server. Retrying connection every %d seconds %d times.\n",
			dj.Config.Connection.Retry.Interval,
			dj.Config.Connection.Retry.Limit)

		success := false
		for retries := 0; retries <= dj.Config.Connection.Retry.Limit; retries++ {
			...
			time.Sleep(dj.Config.Connection.Retry.Interval * time.Second)
		}
		if !success {
			dj.KeepAlive <- true
...
func (dj *MumbleDJ) OnTextMessage(e *gumble.TextMessageEvent) {
	...
	if len(plainMessage) != 0 {
		if plainMessage[0] == dj.Config.CommandPrefix[0] && plainMessage != dj.Config.CommandPrefix {
...
func (dj *MumbleDJ) HasPermission(user *gumble.User, isAdminCommand bool) {
